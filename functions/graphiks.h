...
void drawGhost(struct SDL_Renderer *renderer, struct Ghost *ghost, struct Pacman *pacman)
{
    ...
    if (pacman->isKilling)
    {
        renderImage(renderer, ghost->scaredTile, ghost->x, ghost->y, BLOCK_SIZE, BLOCK_SIZE, 0);
    }
    else
    {
        renderImage(renderer, ghost->tile, ghost->x, ghost->y, BLOCK_SIZE, BLOCK_SIZE, 0);
    }
}
